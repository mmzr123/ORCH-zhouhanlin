# -*- coding: utf-8 -*-
"""
mmlu_router_4choice_k2_ema.py

- 多学科 MMLU 4 选一评测
- ORCH (K=2 全链自洽):
    * 拆解层 K=2：将题目拆成 2 个互补子问题
    * 子问题层 K=2：每个 agent × 每个子问题 采样 2 次，自洽融合为一段分析
    * 合并层 K=2：最终合并调用 2 次，对 A/B/C/D 做多数表决
  * Panel=2：用 EMA 评分选出前 2 个 LLM 进入 panel
  * Merger=评分第 1 的 LLM
...
- 图像输出（在 PLOT_DIR 下）：
  ...
  6) ema_curves.png  （quality & latency 随题号曲线）
"""
...
CFG: Dict[str, Any] = {
    "LLM_TEMPERATURE": 0.0,       # 固定为 0，保证确定性（如需更强自洽多样性可调高）
    ...
    # ====== K 设置：全链统一 K=2 ======
    "K_DECOMP": 2,                # 拆解出 2 个子问题
    "K_SUBQ": 2,                  # 每个 (agent, subq) 采样 2 次自洽
    "K_MERGE": 2,                 # 最终合并采样 2 次表决
    "PANEL_TOPK": 2,              # 选择 2 个 LLM 进入 panel
}
...
PLOT_DIR = "plots_4choice_k2_ema-1111"
...
class OpenAICompatAgent(BaseAgent):
    ...
    def infer(self, req: "OrchestrateRequest") -> AgentResult:
        try:
            ...
            text = f"[{self.name} ERROR] {type(e).__name__}: {e}\nEcho: {req.query[:200]}"
            res = AgentResult(text, latency_ms=1200, http_status=500, error=str(e))
...
class Orchestrator:
    ...
    def _score_agent(self, name: str) -> float:
        ...
        lat = st.get("ema_latency_ms", 2000.0)
        cost = st.get("ema_cost", 0.0)
        stab = st.get("ema_stability", 0.9)

        norm_lat = min(lat / 5000.0, 3.0)
        norm_cost = min(cost / 0.002, 3.0)
        return float(q + 0.1 * stab - 0.3 * norm_lat - 0.1 * norm_cost)

    # ====== K=2: 拆解 2 个子问题 ======
    def _decompose_k2(self, dispatcher: Agent, prompt: str) -> List[str]:
        tpl = (
            "将下面的四选一题拆分为2个互补的分析子问题，覆盖：概念判别/选项排除/推理核验（综合在两条里），"
            "每条不超过40字。只输出两行，每行一个子问题，无其它多余内容。\n\n"
            f"{prompt}\n"
        )
    ...
    def _one_agent_analysis_once(self, agent: Agent, full_prompt: str, subq: str) -> AgentResult:
        ana_tpl = (
    ...
    # ====== K=2: 子问题层自洽 ======
    def _one_agent_analysis_k2(self, agent: Agent, full_prompt: str, subq: str) -> Tuple[str, int]:
        r1 = self._one_agent_analysis_once(agent, full_prompt, subq)
        r2 = self._one_agent_analysis_once(agent, full_prompt, subq)

        a1 = normalize_to_letter_4(r1.result_text)
        a2 = normalize_to_letter_4(r2.result_text)

        final_letter = a1 if a1 == a2 or a1 in LABELS_4 else (a2 if a2 in LABELS_4 else "?")

        fused = (
            f"[SC-1] {truncate_chars(r1.result_text, 240)}\n\n"
            f"[SC-2] {truncate_chars(r2.result_text, 240)}\n\n"
            f"[K=2 自洽投票] {a1}/{a2} -> {final_letter}"
        )
        fused = truncate_chars(fused, CFG["ANSWER_MAX_CHARS"])
        avg_ms = int(mean([r1.latency_ms, r2.latency_ms]))
        return fused, avg_ms

    # ====== K=2: 合并层自洽 ======
    def _merge_k2(self, analyses: List[Tuple[str, str]], full_prompt: str, merger: Agent) -> str:
        bullet = "\n\n".join([f"[{name}] {txt}" for name, txt in analyses])
        merge_tpl = (
            "你将看到多名分析员对同一四选一题的分析（均不超过500字）。"
            "请综合证据，仅输出最终选项字母（A/B/C/D），不得附加解释或其它字符。\n\n"
            f"{full_prompt}\n\n{bullet}\n\n最终答案："
        )
        r1 = merger.infer(OrchestrateRequest(merge_tpl))
        r2 = merger.infer(OrchestrateRequest(merge_tpl))
        a1 = normalize_to_letter_4(r1.result_text)
        a2 = normalize_to_letter_4(r2.result_text)
        return a1 if a1 == a2 or a1 in LABELS_4 else (a2 if a2 in LABELS_4 else "?")

    def orchestrator_answer(self, prompt: str) -> Tuple[str, int]:
        """
        EMA 评分 -> 选 top-2 进 panel；
        拆解 K=2 -> panel×subq 共 4 段分析，每段内部 K=2 自洽；
        最终合并 K=2。
        """
        t0 = time.time()
        ...
        top_k = min(CFG["PANEL_TOPK"], len(scores))
        panel = [a for _, a in scores[:top_k]]
        dispatcher = panel[0]
        merger = panel[0]

        subqs = self._decompose_k2(dispatcher, prompt)

        analyses: List[Tuple[str, str]] = []
        lat_parts: List[int] = []

        with ThreadPoolExecutor(max_workers=len(panel) * len(subqs)) as ex:
            futs = []
            key_map: Dict[Any, Tuple[str, str]] = {}
            for ag in panel:
                for sq in subqs:
                    fut = ex.submit(self._one_agent_analysis_k2, ag, prompt, sq)
                    futs.append(fut)
                    key_map[fut] = (ag.name, sq)
            for fu in as_completed(futs):
                fused_txt, part_ms = fu.result()
                name, _sq = key_map[fu]
                analyses.append((name, fused_txt))
                lat_parts.append(part_ms)

        final_letter = self._merge_k2(analyses, prompt, merger)
...
def mmlu_eval_multi_subject(
    ...
    def record_ema_snapshot():
        for name, st in ema_state.items():
            ...
            qv = float("nan") if q is None else float(q)
            lv = float("nan") if l is None else float(l)
    ...
    ax.set_title("MMLU (4-choice, multi-subject, K=2 self-consistency)")
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Accuracy")
    ax.set_title("Per-Subject Accuracy (ORCH vs baselines + VOTE, K=2)")
    ax.legend()
    fig.tight_layout()
...
def main():
    ap = argparse.ArgumentParser(
        description="MMLU 4-choice multi-subject: ORCH(K=2 self-consistency) + baselines + VOTE + EMA plots"
    )
