...
- 生成 5 张图（在 plots_4choice/ 下）：
  1) global_acc.png                全局 ACC 柱状图
  2) per_subject_acc.png           分学科 ACC 柱状图（ORCH + 3 基线）
  3) orch_confusion.png            ORCH 4×4 混淆矩阵
  4) mcnemar_orch_vs_deepseek.png  McNemar (ORCH vs DEEPSEEK)
  5) price_latency_table.png       价格 + 平均延迟表格
"""
...
def mmlu_eval_multi_subject(
    ...
    for subj in subjects:
        ...
        for j, i in enumerate(idxs, 1):
            ...
            print(
                ...
                f"DEEPSEEK={deepseek_ans} | XAI={xai_ans} | lat={lat_ms}ms"
            )
    ...
    for subj in subjects:
        ...
        per_subject_rows.append((subj, ao, aope, adeep, ax))
        line = (
            f"  {subj:26s} ORCH={ao:.3f}  OPENAI={aope:.3f}  "
            f"DEEPSEEK={adeep:.3f}  XAI={ax:.3f}"
        )
        print(line)
    ...
    names = ["ORCH", "OPENAI", "DEEPSEEK", "XAI"]
    vals = [acc_orch, acc_open, acc_deep, acc_xai]
    ax.bar(names, vals)
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Accuracy")
    ax.set_title("MMLU (4-choice, 10 subjects, ~300 Qs)")
    ...
    ax = fig.add_subplot(1, 1, 1)
    ax.bar(x - 1.5*width, orch_accs, width, label="ORCH")
    ax.bar(x - 0.5*width, open_accs, width, label="OPENAI")
    ax.bar(x + 0.5*width, deep_accs, width, label="DEEPSEEK")
    ax.bar(x + 1.5*width, xai_accs,  width, label="XAI")
    ax.set_xticks(x)
    ax.set_xticklabels(subj_names, rotation=45, ha="right")
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Accuracy")
    ax.set_title("Per-Subject Accuracy (ORCH vs 3 baselines)")
    ax.legend()
    fig.tight_layout()
    ...
    ax.text(0.65, max(b, c)*0.9 if (b or c) else 0.5, text,
            ha="left", va="top")
    fig.tight_layout()
...
def main():
    ap = argparse.ArgumentParser(
        description="MMLU 4-choice multi-subject: ORCH(3-agent analyses + merge) + 3 baselines + 5 plots"
    )
