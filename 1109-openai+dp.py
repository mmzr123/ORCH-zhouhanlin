# -*- coding: utf-8 -*-
"""
mmlu_pro_10choice_orch2_10dirs.py

MMLU-Pro (10-choice A–J), 10 个方向（category），每个方向 30 题：
- 单模型基线：OPENAI / DEEPSEEK / XAI
- VOTE：3 模型直接答 A–J，多数投票
- ORCH-2：只用 OPENAI + DEEPSEEK 写 10 选一分析 + 合并（输出 A–J）

输出：
  控制台：
    - GLOBAL ACC（ORCH-2 + 三基线 + VOTE）
    - PER-DIRECTION ACC（10 个方向）
    - ORCH-2 的 10×10 混淆矩阵
    - McNemar (ORCH-2 vs 最强单模型基线)
    - 平均延迟统计（3 provider + ORCH-2 + VOTE）

  图像（plots_mmlu_pro_orch2_10dirs/）：
    - global_acc.png
...
PLOTS_DIR = "plots_mmlu_pro_orch2_10dirs"
...
def call_chat_api(
    ...
    如果 api_key 为空，则返回 echo；带 requests 超时保护。
    """
    if not api_key:
    ...
    try:
        ...
        dur_ms = int((time.time() - t0) * 1000)
        text = "[TIMEOUT] request exceeded 40s"
        return text, dur_ms
...
# ===================== ORCH-2：OpenAI + Deepseek 分析 + merge =====================
def orch2_answer(prompt: str) -> Tuple[str, int]:
    """
    ORCH-2：
      1) 只用 OPENAI + DEEPSEEK 写 10 选一分析（各一段，≤300 字）
      2) 合并 Agent 也只在两者中选（优先 OPENAI，否则 DEEPSEEK），输出 A–J
    ...
    if not analysis_agents:
        # 两个都没 key，就退化为 OPENAI baseline（如果有的话）或者直接 "?"
        raw, lat = ask_mcq_10choice("openai", prompt) if OPENAI_API_KEY else ("?", 0)
        return raw, lat

    analyses = []
    total_lat = 0

    # step 1: OpenAI + Deepseek 各写一段分析
    ana_tpl = (
    ...
    # step 2: 合并 Agent：只在 OPENAI / DEEPSEEK 中选，优先 OPENAI
    if OPENAI_API_KEY:
...
def plot_global_acc(global_acc: Dict[str, float]):
    models = ["ORCH-2", "OPENAI", "DEEPSEEK", "XAI", "VOTE"]
    ...
    plt.ylim(0, 1.0)
    plt.ylabel("Accuracy")
    plt.title("Global Accuracy (MMLU-Pro, 10 dirs × 30 Q, ORCH-2)")
...
def plot_per_direction_acc(dir_names: List[str],
    ...
    plt.ylabel("Accuracy")
    plt.title("Per-Direction Accuracy (10 dirs, 30 Q each, ORCH-2)")
    plt.legend()
    plt.tight_layout()
...
def plot_orch_confusion(golds: List[str], preds_orch: List[str]):
    ...
    ax.set_ylabel("Gold")
    ax.set_title("Confusion Matrix (ORCH-2, A–J)")
...
def plot_mcnemar_text(best_baseline_name: str, stats: Dict[str, float]):
    ...
    txt = (
        f"McNemar Test: ORCH-2 vs {best_baseline_name}\n\n"
        f"b = ORCH-2 correct & {best_baseline_name} wrong = {b}\n"
        f"c = ORCH-2 wrong  & {best_baseline_name} correct = {c}\n"
        f"b + c = {bc}\n"
        f"chi2 = {chi2:.4f}\n"
        f"p ≈ {p:.6f}\n\n"
...
def plot_price_latency_table(avg_latency: Dict[str, float]):
    """
    价格硬编码 + ORCH-2 / VOTE 的相对价格和平均时延。
    """
    header = ["Model", "In_cached", "In_noncached", "Out", "Avg latency (ms)"]

    rows = [
        [
            "ORCH-2 (OpenAI+Deepseek analysis+merge)",
            "≈ 3×(O + D)",   # 2 段分析 + 1 次合并
            "-",
            "-",
            f"{avg_latency.get('ORCH-2', float('nan')):.1f}",
        ],
        [
            "VOTE (3-model)",
            "≈ O + D + X",
    ...
    ax.axis("off")
    table_data = [["Model", "In_cached", "In_noncached", "Out", "Avg latency (ms)"]] + rows
    table = ax.table(
        cellText=table_data,
        cellLoc="center",
        loc="center",
    ...
    table.scale(1, 1.4)
    ax.set_title("Price & Average Latency (ORCH-2)", pad=10)
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="MMLU-Pro 10-choice, 10 dirs × 30 Q, ORCH-2 (OpenAI+Deepseek) + 3 baselines + VOTE."
    )
    ...
    total_lat_orch2 = total_lat_vote = 0.0
    n_calls_open = n_calls_deep = n_calls_xai = 0
    n_calls_orch2 = n_calls_vote = 0
    ...
    for qi, i in enumerate(indices, 1):
        ...
        # ORCH-2
        orch_ans, lat_orch = orch2_answer(prompt)
        ...
        print(
            f"[Q{qi}/{len(indices)}] GOLD={gold_letter} | "
            f"ORCH-2={orch_ans} | OPENAI={open_ans} | "
    ...
    for d in dir_names:
        ...
        print(
            f"  {d:30s} ORCH-2={acc_orch:.3f}  OPENAI={acc_open:.3f}  "
            f"DEEPSEEK={acc_deep:.3f}  XAI={acc_xai:.3f}  VOTE={acc_vote:.3f}"
        )

    # ===== Confusion Matrix (ORCH-2) =====
    print("\n[CONFUSION MATRIX ORCH-2 (raw counts, A–J)]")
    labels = A_J
    ...
    print(f"\n[MCNEMAR] ORCH-2 vs {best_base}")
    print(f"  b = ORCH-2 correct & {best_base} wrong = {stats['b']}")
    print(f"  c = ORCH-2 wrong  & {best_base} correct = {stats['c']}")
    ...
    avg_latency = {
        ...
        "ORCH-2":  total_lat_orch2 / max(1, n_calls_orch2),
