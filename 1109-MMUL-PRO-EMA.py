# -*- coding: utf-8 -*-
"""
mmlu_pro_10choice_orch_10dirs_ema_router.py

MMLU-Pro (10-choice A–J), 10 个方向（category），每个方向 per_dir 题：
- 单模型基线：OPENAI / DEEPSEEK / XAI
- VOTE：3 模型直接答 A–J，多数投票
- ORCH：多模型分析 + 合并
  * 分析和合并模型通过 EMA 路由选择（ema_quality / ema_latency / ema_cost / ema_stability）
...
输出：
  控制台：
    ...
    - 每个 provider 的最终 EMA 指标
    - 每题的路由器决策日志（调度器内容）

  图像（plots_mmlu_pro_orch_10dirs）共 6 张：
    - global_acc.png
    - per_direction_acc.png
    - orch_confusion.png
    - mcnemar_orch_vs_best.png
    - price_latency_table.png
    - ema_quality_curve.png   （按题号画出 3 agent 的 EMA accuracy 曲线）
"""
...
PLOTS_DIR = "plots_mmlu_pro_orch_10dirs-1110"
...
PRICE_PER_M = {
    ...
    "DEEPSEEK": {
        "currency": "CNY",
        "input": 2.0,
        "output": None,   # 若无 output 价，就用 input 价
    },
    "XAI": {
        "currency": "USD",
        "input": 2.0,
...
def ema_routing_key(agent_name: str,
    ...
    """
    返回用于排序的 key：
      (-quality, latency, cost, -stability)
    None 用默认值做冷启动：
...
def orch_answer(prompt: str,
    ...
    """
    ORCH（带 EMA 路由）：
      1) 根据 EMA（质量 / 延迟 / 成本 / 稳定性）对 OPENAI / DEEPSEEK / XAI 排序；
      2) 选出最多 3 个作为分析 Agent；
      3) 取排序第一的作为合并 Agent；
      4) 输出最终 A–J。
    ...
    # 合并 Agent 取 score 最优者
    merger = candidates_sorted[0]
    analysis_names = [a[0] for a in analysis_agents]
    merger_name = merger[0]
    print(f"[ROUTER] chosen analysis panel = {analysis_names}, merger = {merger_name}")
